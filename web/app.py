...
def create_web_app(get_pm):
    ...
    # --- PLAY (corrigé pour boucle Discord) ---
    ...
    def api_play():
        ...
        try:
            # Le vrai fix : on lance la coroutine sur la loop du bot Discord
            fut = asyncio.run_coroutine_threadsafe(
                music_cog.play_for_user(guild_id, user_id, {"title": title, "url": url}),
                app.bot.loop
            )
    ...
    def api_resume():
        ...
        try:
            fut = asyncio.run_coroutine_threadsafe(
                music_cog.resume_for_web(guild_id),
                app.bot.loop
            )
            fut.result(timeout=30)
        except Exception as e:
    ...
    def api_stop():
        ...
        try:
            fut = asyncio.run_coroutine_threadsafe(
                music_cog.stop_for_web(guild_id),
                app.bot.loop
            )
            fut.result(timeout=30)
        except Exception as e:
    ...
    def api_skip():
        ...
        try:
            fut = asyncio.run_coroutine_threadsafe(
                music_cog.skip_for_web(guild_id),
                app.bot.loop
            )
            fut.result(timeout=30)
        except Exception as e:
